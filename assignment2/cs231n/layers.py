...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    ###########################################################################
    mu = 1 / float(D) * np.sum(x, axis=1)[:, np.newaxis]
    xmu = x - mu
    carre = xmu ** 2
    var = 1 / float(D) * np.sum(carre, axis=1)
    sqrtvar = np.sqrt(var + eps)
    invvar = 1. / sqrtvar
    va2 = xmu * invvar[:, np.newaxis]
    va3 = gamma * va2
    out = va3 + beta
...
def layernorm_backward(dout, cache):
    ...
    # still apply!                                                            #
    ###########################################################################
    mu, xmu, carre, var, sqrtvar, invvar, va2, va3, gamma, beta, x, ln_param = cache
    eps = ln_param.get('eps', 1e-5)
