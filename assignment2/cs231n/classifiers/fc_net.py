...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        a_r1, cache_a_r1 = affine_relu_forward(
          X, self.params['W1'], self.params['b1'])
        scores, cache_scores = affine_forward(
          a_r1, self.params['W2'], self.params['b2'])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        _, dw1, db1 = affine_relu_backward(dx, cache_a_r1)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for l in range(self.num_layers):
          i = l + 1
          w_dim = (layer_dims[l], layer_dims[i])
          b_dim = layer_dims[i]
          self.params['W' + str(i)] = np.random.normal(0, weight_scale, w_dim)
          self.params['b' + str(i)] = np.zeros(b_dim)
