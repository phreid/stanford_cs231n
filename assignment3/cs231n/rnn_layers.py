...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for step in range(t):
        x_step = x[:, step, :]
        next_h, next_cache = rnn_step_forward(x_step, prev_h, Wx, Wh, b)
        h[:, step, :] = next_h
...
def rnn_backward(dh, cache):
    ...
    for step in reversed(range(t)):
        ...
        dx_step, dprev_h_step, dWx_step, dWh_step, db_step = \
            rnn_step_backward(dh_step, cache_step)
        dx[:, step, :] = dx_step
        dh0 = dprev_h_step
        dWx += dWx_step
        dWh += dWh_step
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for step in range(t):
        x_step = x[:, step, :]
        next_h, next_c, next_cache = lstm_step_forward(x_step, prev_h, prev_c, Wx, Wh, b)
        h[:, step, :] = next_h
        cache.append(next_cache)
        prev_h = next_h
        prev_c = next_c
...
def lstm_backward(dh, cache):
    ...
    for step in reversed(range(t)):
        ...
        dx_step, dprev_h_step, dprev_c_step, dWx_step, dWh_step, db_step = \
            lstm_step_backward(dh_step, dprev_c_step, cache_step)
        dx[:, step, :] = dx_step
        dh0 = dprev_h_step
        dWx += dWx_step
        dWh += dWh_step
